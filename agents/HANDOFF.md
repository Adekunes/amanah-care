...
Recommendations for today (owner's spec, 01:30): `app/web/recommendations.json` (13 rows, strict schema, leading block comment lists rows to review; `parseTable()` strips it), `app/web/patterns.js` (pure engine: `detect(table, ctx)`, `fastingToday(prefs, now)`, `AVAILABLE` signals; one detector per starter pattern; rows on unconnected signals and unknown ids skipped; meal patterns suppressed on a stored fasting day; acknowledged patterns skipped until `until`; sorted contact, attention, notice), `app/web/recs.js` (sibling module on `window.amanah`: `onHome` hook, getters prefs/routine/weekItems/handoffs/members/elderName/planFor/doneFor/isServerView, `postEncEvent`; fetches 14 days, decrypts, runs detect, renders max 5 blocks with tier pill, because line, role actions, cultural note, escalation with the stored care contact, provenance, "Acknowledge for 24 h" which posts an encrypted `PatternAcknowledged {pattern_id, until}`; hidden for the elder and in Server view), `PatternAcknowledged` in LISTABLE, Prefs gained `fasting` and `care_contact`, `app/test/patterns.test.js` (table lint for the hard constraints, every detector fires and does not, fasting and acknowledgement suppression, unconnected signals, ordering), seed shifted so Sister A carries 75% of the week, lunch refused on the last two days, no mobility logged for 48 h.

Hospital sheet, patterns, emergency (00:30, owner's idea): Prefs gained conditions, allergies, doctor, contacts (same encrypted PreferenceSet). `openSheet()` renders `#view-sheet` from LOG_ITEMS (14 days), HANDOFFS (last 3), ROUTINE (upcoming appointments), PREFS, with print CSS (`Print / save PDF` calls window.print()). `computePatterns(days)` returns factual count lines (mood words by day with "every time after 18:00", refused food, ate half, meds skipped, walks), shown on Home and on the sheet. Emergency: `EmergencyRaised` event (no payload); `raiseEmergency()` confirms, posts, opens the sheet; other phones get `showEmergencyBanner()` from the live stream (`#emg-banner`, sticky red, "Hospital sheet" / "Dismiss"); the elder screen has a big "I need help" button; the notifier's `emergency` kind reaches every member regardless of subscriptions (see spec §1). Seed: hospital fields in PreferenceSet, evening "agitated" on three days, refused lunch on two.
