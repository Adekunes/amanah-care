...
8. Log in as `layla` (password 333) in a fourth tab to show the picker: two families open, pick Ammi's. Point out the blue dashboard, "Your shift with Ammi", no Invite tab. On the Log tab pick two presets at once and log them, then Undo one from the toast; the record keeps it, struck through, marked retracted.
