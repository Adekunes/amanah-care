...
- **Desktop layout**: same DOM, a CSS media query turns the tab row into a left sidebar and stacks Home into two columns with the hero full width, and Log / Hand off / Routine / Alerts / Flow into two columns, and the landing into three cards side by side; the elder view stays one centred column; phones unchanged. `app/web/styles.css` (`@media (min-width:960px)` block).
- **Log page**: 11 categories (meds, meal, prayer, mobility, sleep, personal care, readings, appointment, transport, note), 4 to 8 presets each, a note hint per category (readings take the number), several presets logged at once, each its own `CareLogged` event so counts and patterns stay exact. `app/web/app.js` (`CATS`, `NOTE_HINT`, `pick.presets`, `logItem()`).
- **Undo**: a mistaken log is retracted by a `CareRetracted` event that points at the original through the `handoff_id` routing field, no deletion, no payload; the toast after logging offers Undo for 8 seconds, a caregiver's own items from the last 24 hours show an undo link in the record, and a retracted row stays in the log struck through ("retracted · undone by <name>") but drops out of counts, the dashboard, patterns, recommendations, the hospital sheet and the workload view. `app/web/app.js` (`retractCare()`, `toastAction()`), `app/api/app.js` (`GET /families/:id/care`), `app/db/init.sql` (`workload_view`).
- **Roles**: family members are green, support workers are blue (`body.support` swaps the palette), a support worker's Home reads "Your shift with Ammi" and "Your visit today" in place of "Ammi's day" and "Your part today", support workers get no Invite tab, and recommendations show the professional actions for their role; the elder screen is unchanged. `app/web/app.js` (`setTabsForRole()`, `SUPPORT_HIDE`), `app/web/styles.css` (`body.support`), `app/web/recs.js` (role branch on `recommendation_pro`/`recommendation_family`).
- **One login, several families**: `logins` is keyed by `(login, family_id)`; a second family registers under the same login only with the same password (403 otherwise); `/auth/login` returns every family the login opens (`families[]`, oldest first) plus the first at top level for older clients; the phone unwraps each family key with the password, names each family from its own encrypted `FamilyCreated` event, shows a picker when there is more than one, keeps the unwrapped keys in the tab session, and offers "Switch family" in the identity bar. `app/db/init.sql` (`logins`), `app/api/app.js` (`/auth/register`, `/auth/login`), `app/web/app.js` (`enterFamily()`, `showFamilyPicker()`, `switchFamily()`).
- **Login page**: three doors, log in, create your account with a family code, or start a new family, with the family code explained as generated by whoever started the family, carrying the family key, to be handed over face to face. `app/web/index.html` (landing section).
- **Seed**: a support worker, Nurse Layla (login `layla`, password 333), in Ammi's family with personal-care and readings logs, and a second family, the Khan family (elder Nana, member Yusuf, login `yusuf`), where Layla is also the support worker; Sister A carries 80% of Ammi's week, lunch is refused on the last two days, and no mobility is logged for 48 hours. `app/seed/seed.js`.

Covered by `app/test/multifamily.test.js` (same-password rule for a second family, one login per member inside a family, family list ordering oldest first, single-family login stays a one-element list). 119 tests.

---

- **Business model (owner's spec, 10:05):** `pitch/BUSINESS.md` (French, English summary at the end). Positioning; pricing grid (families free and unlimited, voluntary support $3 to $5 for extras only, agencies and care organisations $8 to $15 per elder per month, community organisations about $600 a year for 50 sponsored families, self-hosting open source with support from $150 a month); capacity math for the 1 million CPU-hours a year target (114 cores; 19,000 to 150,000 active families, estimates); break-even N = C ÷ p (84 elders cover the $1,000 server at $12, about 420 cover $5,000); retention levers; what we will not do; risks. Deck slide 9, backup A3, qa.html and explainer §12 follow it.

- **User research (10:50):** `pitch/RESEARCH-FORUMS.md`, 35 verified pages from public caregiver communities (AgingCare, AlzConnected, Alzheimer's Society UK forum, Mumsnet, ISPU, MuslimMatters, Amaliah, Caring.com), six angles, every page re-opened before counting; pain-point table, three "in their words" lines for Box 2, ranked gaps. `pitch/RESEARCH-REDDIT.md` records that Reddit was unreachable (fetch blocked, old.reddit requires login, extension dropped); nothing is attributed to Reddit. Slide 4, the Q&A "research" answer, the sources slide and SCRIPT.md Box 4 cite the forum note.

- **The board (branch `kanban`, 2026-09-06 11:45, owner's spec):** Home is a three-column board, To do / Done / Blocked, with five cards a day (Medication, Personal care, Meals, Prayers, Walk; `web/board.js` `DEFAULT_CARDS`, the seed routine, `RoutineSet` for a new family). Done = `CareLogged` (unchanged, so workload, record, handoff, hospital sheet and rules all keep working), Blocked = new `CareBlocked` event (`{text, reason, routine_id}` encrypted, `category` clear; in `LISTABLE`, in the notifier's `kindsFor`, returned by `GET /families/:id/care` with its `type`), back to To do = `CareRetracted` pointing at the move. `workload_view` now self-joins so undoing a Blocked card costs no credit. Board state per day is computed on the phone (`boardFor()` in `web/board.js`, local midnight, latest non-retracted event per card wins), so the daily reset is free and the history stays. Drag and drop on desktop, Done / Blocked / To do buttons on every card, a small "why not?" sheet (refused, asleep, agitated, other) before a Blocked move, toast with Undo. The old Log tab is out of the tab row (its code stays behind `#tab-log`); recommendations, "patterns not predictions", your part, last 7 days and who carried the week moved to a **Patterns** tab. Elder view lists the five cards read-only. Rules: `MOOD_AGITATED_EVENINGS` also reads a blocked card's reason; a blocked Meals card reads as "refused". `dev/stack.mjs` injects `window.AMANAH_API` when `PORT` is not 4000 (a second stack next to Docker). Tests: `test/board.test.js`, CareBlocked in `api.test.js`, `notifier.test.js`, `patterns.test.js`. 130 tests. **Docker note:** `init.sql` changed (`workload_view`); a Postgres volume created before this needs `docker compose down -v` before `up --build`.

- **Board upgrades (12:30):** add a card from the To do column (label, kind, time, who; saved into `RoutineSet`); tap a card for its sheet (what, when, kind, who, last 7 days as coloured squares, Remove); hold-and-drag on touch (`touchDrag()`), HTML5 drag on desktop; "late" pill on a To do card past its time by an hour; Everyone / Mine filter; day pager to look at yesterday and the last six days, read only (`boardDay`). Family tab: last move, days active out of 7, call link from the stored contacts, assign or unassign any card by tapping it on the member, and a week heatmap (cards done per person per day). `app/web/app.js` (`openCard()`, `touchDrag()`, `renderFamily()`), `index.html` (`#cardsheet`, `#fam-heat`).
...
- `CareRetracted` (in LISTABLE) lets a member undo a mistaken `CareLogged` without deleting it: routing fields only (family_id, type, actor_id = who undid it, occurred_at), no payload. It reuses the `handoff_id` column to hold the id of the `CareLogged` event being retracted, not a handoff id (no schema change; documented at the read site in `api/app.js` and `db/init.sql`).
- `GET /families/:id/care` LEFT JOINs a `GROUP BY handoff_id` subquery of `CareRetracted` rows onto `CareLogged` rows to add `retracted`/`retracted_by` without duplicating a row when it is retracted twice (pg-mem does not reliably run EXISTS subqueries, so this avoids that).
- `workload_view.care_count` nets `count(CareLogged) - count(CareRetracted)` per family/actor/day, so retracting a log removes its credit instead of just hiding it.
...
3. A: Home is the board: five cards, To do / Done / Blocked. Drag Meals to Done (desktop) or press Done on the card (phone). Press Blocked on Walk, pick "agitated": the card lands in Blocked with the reason. "2 of 5 done" moves. Nothing else on Home.
...
8. Log in as `layla` (password 333) in a fourth tab to show the picker: two families open, pick Ammi's. Point out the blue board, "Your shift with Ammi", no Invite tab. Move Personal care to Done, then Undo from the toast; the record keeps the move, struck through, marked retracted. Patterns tab: the recommendations (Meals refused twice, agitated evenings, no walk in 48 h, Sister A carries 80%).
9. Tests: `npm run coverage` on the terminal for the coverage table.
